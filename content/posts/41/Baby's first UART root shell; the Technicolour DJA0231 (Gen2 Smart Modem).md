...
Some more techniques for determining what different UART ports look like across different device types can be seen [here](https://www.youtube.com/watch?v=s8s3gvZPc0c), and is a good reference. However, **safely powering it on & probing around can be the most helpful **

### Finding the `TX` pin

To interface with/connect to the router's serial interface, and all of its juicy debug logs + a potential shell, we need to identify which pins are which (`TX`, `GND` and `RX`) on the router's suspected UART interface.

1. Set a multimeter to the `20V` setting (i.e. it will read up to `20V`, which should encompass the range including the expected `3.3V`ish output of the pins). Then, with the meter's black wire/pin grounded (i.e. touching some form of metal), probe each pin on the suspected UART line to read the voltage, directly after powering the device on.
2. The `GND` (Ground) pin should read **`0V.`**
3. The `TX` pin *might* be located on a pin/pad that has a **fluctuating/unsteady voltage** reading, indicating the (expected) variations in data being transmitted as the router powers on & generates boot logs over serial, etc. As these logs are all **communicated over the serial wire as max/min voltage ==pulses==**, which correspond to encoded `1s` and `0s`, any pin that has **some kind of pulsating/fluctuating voltage reading is likely the one actively transmitting data**. 
4. [OPTIONAL/AFTERWARDS] Once the `TX` is found, later on in the guide when you're connected and have serial logs coming through on your laptop/a shell, you can find the `RX` pin by probing the remaining unknown pins & spamming your keyboard, to see which one allows data to be received by the device.
...
Determine the `/dev/` name of the USB-to-UART serial device, when it's plugged into your computer of choice. This can be done in numerous ways, but I was doing this on an M1 Mac, so was a little roundabout compared to the typical linux `lsusb` (or running `dmesg -w` and checking the new line that appears when plugged in). These *may* also work on a mac as well, but did not for me, so I followed the [below method](https://stackoverflow.com/questions/48291366/how-to-find-dev-name-of-usb-device-for-serial-reading-on-mac-os):
...
```
![](ATTACHMENTS/Screenshot%202026-07-22%20at%2010.22.41%20pm.png)
So, `/dev/tty.usbserial-1230` is the device's `/dev/` entry, requried to read the serial output!
...
# Using our discovered plugged in USB-to-UART mount point
...
Going forward, the sky's the limit, and I'm *very* much still learning & *very* much am eager to do so. <3